...
function cmake.header(prj)
    ...
    _p('cmake_minimum_required(VERSION 3.10)')
...
function cmake.cfgRules(cfg, str)
...
function cmake.tablefind(tab, el)
...
function cmake.project(prj)
    ...
    for _, cfg in ipairs(configurations) do
        ...
        cmake.cfgRules(cfg.includedirs, includestr)

        -- add build defines
        cmake.cfgRules(cfg.defines, definestr)

        -- set MSVC build type on Windows
        if os.is("windows") then
            if premake.config.isdebugbuild(cfg) then
                _p(1, 'set(CMAKE_MSVC_RUNTIME_LIBRARY \"%s\")', iif(cfg.flags.StaticRuntime and not cfg.flags.Managed, "MultiThreadedDebug", "MultiThreadedDebugDLL"))
            else
                _p(1, 'set(CMAKE_MSVC_RUNTIME_LIBRARY \"%s\")', iif(cfg.flags.StaticRuntime and not cfg.flags.Managed, "MultiThreaded", "MultiThreadedDLL"))
            end
        end

        if os.is("windows") then
            local msvc_c_flags = cmake.list(table.join(cc.getcppflags(cfg), cc.getcflags(cfg), cfg.buildoptions, cfg.buildoptions_c))
            local msvc_cxx_flags = cmake.list(table.join(cc.getcppflags(cfg), cc.getcflags(cfg), cc.getcxxflags(cfg), cfg.buildoptions, cfg.buildoptions_cpp))

            -- Remove flags that are not compatible with MSVC toolkit
            msvc_c_flags = msvc_c_flags:gsub("-Wextra ", "")
            msvc_cxx_flags = msvc_cxx_flags:gsub("-Wextra ", "")

            _p('if(MSVC)')

            -- set C flags
            _p(1, 'set(CMAKE_C_FLAGS \"${CMAKE_C_FLAGS} %s\")', msvc_c_flags)

            -- set CXX flags
            _p(1, 'set(CMAKE_CXX_FLAGS \"${CMAKE_CXX_FLAGS} %s\")', msvc_cxx_flags)
    ...
    for _, cfg in ipairs(configurations) do
        ...
        if (prj.kind == 'ConsoleApp' or prj.kind == 'WindowedApp') then
            ...
            _p(1, 'target_link_libraries(%s %s %s %s %s)', premake.esc(cfg.buildtarget.basename), libdirs, cmake.list(cc.getldflags(cfg)), cmake.list(premake.esc(premake.getlinks(cfg, "siblings", "basename"))), cmake.list(cc.getlinkflags(cfg)))
            _p(1, 'target_link_options(%s %s)', premake.esc(cfg.buildtarget.basename), cmake.listWrapped(cfg.linkoptions, " PRIVATE ", ""))
